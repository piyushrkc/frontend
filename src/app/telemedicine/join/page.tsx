...
export default function JoinTelemedicineSession() {
  ...
  const submitFeedback = async () => {
    try {
      ...
      // In development mode, just simulate a successful submission
      if (process.env.NODE_ENV === 'development') {
        console.log('Development mode: Submitting feedback', {
          sessionId,
          rating,
      ...
      // Production code
      const response = await fetch(`/api/telemedicine/sessions/${sessionId}/feedback`, {
        method: 'POST',
        headers: {
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-gray-100">
        <div className="text-center max-w-lg p-6 bg-white shadow-md rounded-lg">
          ...
          <Link href="/telemedicine/dashboard" className="mt-4 inline-block px-4 py-2 bg-primary-600 text-white rounded-md hover:bg-primary-700">
            Return to Dashboard
          </Link>
        </div>
      </div>
